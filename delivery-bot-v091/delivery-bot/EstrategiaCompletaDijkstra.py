...
class SmartPlayer(BasePlayer):
    ...
    def _evaluate_best_goal(self, world, current_steps, maze_solver):
        ...
        for goal in world.goals:
            ...
            dist_to_goal = len(maze_solver.dijkstra(self.position, goal["pos"]))
    ...
    def _evaluate_best_package(self, world, current_steps, maze_solver):
        ...
        for package in world.packages:
            dist_to_pkg = len(maze_solver.dijkstra(self.position, package))
...
class Maze:
    ...
    def dijkstra(self, start, goal):
        ...
        # Dicionários para rastrear o caminho e as distâncias
        came_from = {}
        distances = {tuple(pos): float('inf') for row in range(size) for pos in zip(range(size), [row]*size)}
        ...
        while oheap:
            ...
            for dx, dy in neighbors:
                ...
                # Verifica se o vizinho é um obstáculo (parede)
                if maze[neighbor[1]][neighbor[0]] == 1:
                    continue

                # Calcula a nova distância (custo de cada passo é 1)
                new_dist = distances[current_node] + 1

                # Se um caminho mais curto para o vizinho foi encontrado
    ...
    def game_loop(self):
        while self.running:
            ...
            self.path = self.dijkstra(self.world.player.position, self.current_target)
            if not self.path:
