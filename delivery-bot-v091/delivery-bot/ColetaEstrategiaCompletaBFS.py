...
class SmartPlayer(BasePlayer):
    """
    Implementação de um jogador inteligente que usa greedy_bfs para suas avaliações.
    """
    def __init__(self, position):
    ...
    def _evaluate_best_goal(self, world, current_steps, maze_solver):
        ...
        for goal in world.goals:
            path_to_goal = maze_solver.greedy_bfs(self.position, goal["pos"])
    ...
    def _evaluate_best_package(self, world, current_steps, maze_solver):
        ...
        for package in world.packages:
            path_to_pkg = maze_solver.greedy_bfs(self.position, package)
...
class Maze:
    ...
    def game_loop(self):
        while self.running:
            ...
            path = self.greedy_bfs(self.world.player.position, self.current_target)
            
            if not path and self.world.player.position != self.current_target:
                if self.verbose: print(f"[INFO] Alvo {self.current_target} inacessível com GreedyBFS. Reposicionando.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arquivo_saida", type=str, default="simulacao-SmartPlayer-Greedy.csv", help="Nome do arquivo CSV de saída (Agente Inteligente com Greedy BFS).")
    ...
    parser.add_argument('--timeout', type=int, default=30, help="Tempo máximo em segundos que cada simulação pode durar.")
    ...
    with open(args.arquivo_saida, 'a', newline='') as csvfile:
        ...
        print(f"Executando {num_runs} simulação(ões) com SmartPlayer (Greedy). Timeout: {TIMEOUT_SECONDS}s. Saída: '{args.arquivo_saida}'...")
