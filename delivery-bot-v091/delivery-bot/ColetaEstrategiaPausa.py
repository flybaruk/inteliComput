...
class DefaultPlayer(BasePlayer):
    """
    Implementação padrão do jogador. Escolhe o alvo mais próximo.
    """
    def escolher_alvo(self, world, current_steps):
        sx, sy = self.position
    ...
    def game_loop(self):
        while self.running:
            ...
            if self.world.player.position == self.current_target:
                if self.current_target in self.world.packages:
                    if self.world.player.cargo < 1: # Capacidade de carga do DefaultPlayer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arquivo_saida", type=str, default="simulacao-DefaultPlayer-Pausa.csv", help="Nome do arquivo CSV de saída (Agente Padrão com A*).")
    ...
    with open(args.arquivo_saida, 'a', newline='') as csvfile:
        ...
        print(f"Executando {num_runs} simulação(ões) com DefaultPlayer (A*). Timeout: {TIMEOUT_SECONDS}s. Saída: '{args.arquivo_saida}'...")
